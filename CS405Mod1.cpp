//Morgan Masapollo CS405 Module 1 Assignment
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        if (start + (increment * steps) > std::numeric_limits<T>::max()) //This will check over each incrememnt for overflow/underflow and if deteted will return -1.
        {
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (start - (decrement * steps) < std::numeric_limits<T>::min()) //This will check every decrement for under/overflows and return them to -1.
        {
